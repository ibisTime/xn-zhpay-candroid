...
public class DiscountShopAdapter extends BaseAdapter {
    ...
    public DiscountShopAdapter(Context context, List<ShopDetailsModel.StoreTicketsBean> list, String name, String phone) {
        this.list = list;
        this.name = name;
        this.phone = phone;
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        if (view == null) {
            view = LayoutInflater.from(context).inflate(R.layout.item_discount_shop, null);
    ...
    private void setView(int i) {
        if (list.get(i).getStatus().equals("91")) { // 已过期
            ...
            holder.txtName.setTextColor(context.getResources().getColor(R.color.graycc));
            holder.txtTime.setTextColor(context.getResources().getColor(R.color.graycc));
        }else if(list.get(i).getStatus().equals("1")){ // 已上架
            ...
            holder.txtPhone.setTextColor(context.getResources().getColor(R.color.fontColor_gray));
        ...
        holder.txtKey1.setText("满" + MoneyUtil.moneyFormatDouble(list.get(i).getKey1()) + "元可用");
        holder.txtKey2.setText(((int)list.get(i).getKey2()/1000)+"");

        holder.txtName.setText(name);
        holder.txtPhone.setText(phone);
        SimpleDateFormat s = new SimpleDateFormat("yyyy-MM-dd");
        Date d5 = new Date(list.get(i).getValidateEnd());
