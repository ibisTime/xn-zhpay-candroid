...
public class EvaluateAdapter extends BaseAdapter {
    ...
    private void setView(int position) {
        ...
        ImageUtil.glide(list.get(position).getInteracterUser().getPhoto(),holder.imgPhoto,context);
        holder.txtName.setText(list.get(position).getInteracterUser().getNickname());

        if(list.get(position).getEvaluateType()!=null){
            if(list.get(position).getEvaluateType().equals("A")){
            ...
            }else if(list.get(position).getEvaluateType().equals("B")){
