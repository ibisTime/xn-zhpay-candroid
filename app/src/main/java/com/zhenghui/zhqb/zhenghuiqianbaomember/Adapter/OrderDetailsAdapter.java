...
public class OrderDetailsAdapter extends BaseAdapter {
    ...
    public OrderDetailsAdapter(Context context, List<OrderModel.ProductOrderListBean> list) {
        this.list = list;
    ...
    public void setView(int position) {
        ...
        if(list.get(position).getIsComment().equals("0")){ // 未评价
    ...
    private void evaluate(String evaluateType,int position) {
        ...
        try {
            object.put("orderCode", list.get(position).getOrderCode());
            object.put("jewelCode", list.get(position).getProductCode());
